...
- **Scan-Ergebnisse (Mobile):** Horizontale **Chip-Navigation** statt Ansichten-Dropdown (`ResultsMobileViewNav`); Quick-Links in der Übersichtskarte (`ResultsOverviewQuickLinks`); Issue-Liste als **Karten/Accordion** statt 6-Spalten-Tabelle (`ScanIssueList` → `ScanIssueItem` unter `md`); ab Tablet virtuelle Tabelle; Issue-Filter als Selects in „Liste & Details“; PDF/Gerät in der Scan-Karte.
